from iaTools import pdf2txt
from iaTools import ai_provider as ai_provider
import json
import re
...
def generateQuiz(feedback = None, save = True, pdfRoute = None):
    if pdfRoute:
        pdfFile = pdfRoute
    else:
        pdfFile = testPdfFile
    ...
    try:
        ...
        if save == True:
            with open('results/quiz.json', 'w') as file:
                file.write(clean_text) 
            
        return quiz, clean_text
...
def generateFeedback(quiz: Quiz, responses: List[int]):
    ...
    response = ai_provider.client.models.generate_content(
        model=IA_MODEL, # O gemini-3-flash-preview / gemma-3-27b-it / gemma-4-31B-it
        contents=prompt,
    )
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if len(inp) == 0 or inp.strip().lower() == 'y':
            quiz = generateQuiz(feedback, False)[0]
        else:
